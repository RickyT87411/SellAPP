...
define("_NEXT_BARCODE_PROC_PROD_", "tools_product_setup_next_barcode");
define("_NEXT_BARCODE_PROC_DEV_", "tools_product_setup_next_barcode");
...
$app->get(
    ...
    function ($request, $response, $args) use ($app) {     
        ...
        $proc 	= !$devmode ? _NEXT_BARCODE_PROC_PROD_ : _NEXT_BARCODE_PROC_DEV_;
        ...
        try {
			...
			$stmt = $conn->prepare("CALL $proc(@next_barcode);");
			$stmt->execute();
			$stmt->closeCursor();

			// Capture output parameter
			$next_barcode = $conn->query("select @next_barcode as barcode")->fetch(PDO::FETCH_ASSOC);
			
			return 	$response->withStatus(200)
						->withHeader("Content-Type","application/json")
						->write(json_encode($next_barcode));
