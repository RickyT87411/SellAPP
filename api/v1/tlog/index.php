...
$app->get(
    ...
    function ($request, $response, $args) use ($app) {
    	...
    	try {
			...
			$results = _POST($url, $data, null, $headers, false, false, $errs, $info);
...
$app->post(
    ...
    function ($request, $response, $args) use ($app) {
    	...
    	try {
    		...
    		$content_transfer_encoding = $request->getHeader("Content-Transfer-Encoding");
    		$content_transfer_encoding = is_array($content_transfer_encoding) ? $content_transfer_encoding[0] : null;
			...
			$conn = new PDO("mysql:host=$host;dbname=$dbname", $usr, $pwd, array(PDO::ATTR_PERSISTENT => true, PDO::ATTR_TIMEOUT => 300));
