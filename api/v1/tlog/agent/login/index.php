...
define("_POST_PROC_", "`tlog_agent_signin`");
...
$app->post(
    ...
    function ($request, $response, $args) use ($app) {
    	...
    	global $_TLOG_AGENT_LOGIN_OBJECT_;			
    	...
    	try {
			...
			$stmt->bindParam(":_username", $data[$map["_username"]], PDO::PARAM_STR);
			$stmt->bindParam(":_passphrase", $data[$map["_passphrase"]], PDO::PARAM_STR);
			$stmt->execute();
			...
			$result = $conn->query("select @session_id as session_id;")->fetch(PDO::FETCH_ASSOC);

			$session_id = $result && array_key_exists("error_code", $result) ? $result["error_code"] : null;
