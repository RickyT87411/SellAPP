...
define("_GET_INTEGRATION_LAST_TRANSACTION_PROC_PROD_", "integration_report_integration_last_transaction");
define("_GET_INTEGRATION_LAST_TRANSACTION_PROC_DEV_", "integration_report_integration_last_transaction");
    ...
    function ($request, $response, $args) use ($app) {
    	...
    	try {
			...
			$data = array (
				...
				,"_include_failures"			=>	$request->getParam($_FIELD_MAPPING_["_include_failures"])
			);
			...
			$proc 	= !$devmode ? _GET_INTEGRATION_LAST_TRANSACTION_PROC_PROD_ : _GET_INTEGRATION_LAST_TRANSACTION_PROC_DEV_;
			...
			$stmt->bindParam(':_include_failures', 			$data["_include_failures"]); 
			$stmt->execute();
			// Parse record set
