...
CellEditor.prototype.edit = function(rowIndex, columnIndex, element, value) 
{
	...
	element.classList.add("editablegrid-editting");
...
TextCellEditor.prototype.getEditor = function(element, value)
{
	...
	htmlInput.setAttribute("style", "overflow:hidden; resize: none;");
	if (this.maxLength > 0) htmlInput.setAttribute("maxlength", this.maxLength);

	if (this.fieldSize > 0) {
		htmlInput.setAttribute("size", this.fieldSize);
	} else {
		var autoWidth = parseInt(this.editablegrid.autoWidth(element));
		htmlInput.style.width = autoWidth + 'px'; // auto-adapt width to cell, if no length specified 
	}
