...
$headers = isset($_GET['headers']) ? json_decode($_GET['headers']) : null;
$request = $_POST;
...
if ( !$url ) {
  ...
  // Process a non-GET request
  if ( strcasecmp($_SERVER['REQUEST_METHOD'], 'GET') != 0 ) {
  	$post_fields = array();

  	// Setup for accepted request methods
  	switch ( $_SERVER['REQUEST_METHOD'] ) {
  		case strcasecmp($_SERVER['REQUEST_METHOD'],'POST') == 0:
			parse_str( file_get_contents( 'php://input' ), $post_fields );
			curl_setopt( $ch, CURLOPT_POST, true );
			break;

		case strcasecmp($_SERVER['REQUEST_METHOD'],'DELETE') == 0:
			parse_str( file_get_contents( 'php://input' ), $post_fields );
			curl_setopt( $ch, CURLOPT_CUSTOMREQUEST, 'DELETE' );
			break;

		case strcasecmp($_SERVER['REQUEST_METHOD'],'PUT') == 0:
			parse_str( file_get_contents( 'php://input' ), $post_fields );
	...
	if ( isset($_GET['json']) ) {
    	...
    	curl_setopt( $ch, CURLOPT_POSTFIELDS, strtolower( $post_fields ));
    }
...
if ( isset($_GET['mode']) && $_GET['mode'] == 'native' ) {
  ...
  // Propagate all HTTP request into the JSON data object.
  if ( isset($_GET['full_request']) ) {
  ...
  if($data['status']['http_code'] === 0 || $data['status']['http_code'] === 'ERROR') {
   	$data['status']['error'] = $errors;
   	$data['status']['http_code'] = 500;
  }
